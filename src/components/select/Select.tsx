...
const Select = (
  props: SelectProps &
    Omit<React.ComponentProps<"input">, "defaultValue" | "size">
) => {
  const { options, isDisabled, size, defaultValue, label, error, ...rest } =
    props;
  ...
  return (
    <div>
      {label && <label htmlFor={label} className={mergeClassNames(classes.label)}>{label}</label>}
      <div
